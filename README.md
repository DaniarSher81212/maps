...
Версия: **3.1.0** | Дата: **2026-05-31** | Python **3.11+** | PostgreSQL **15+**
...
Скрипт сам определит окружение (Docker или локальный Python) и запустит систему.  
...
> Подробные инструкции по развёртыванию на корпоративном сервере: [`docs/РАЗВЕРТЫВАНИЕ.md`](docs/РАЗВЕРТЫВАНИЕ.md)
...
```
1. Перечень работ (works)             — сначала: устанавливает даты, наименования, оргструктуру
2. Потребности (requirements)         — материалы для каждой работы
3. Аварийные работы (emergency)       — если есть (тот же формат, что и «Перечень работ»)
4. Потребности аварийных (emergency-requirements) — материалы аварийных работ
5. Складские остатки (stock)          — что есть на складах
6. Поставки (supplies)                — что ожидается
7. Списания (writeoffs)               — что уже на объекте (документально)
8. Выдано не списано (issued)         — что выдано, но не оформлено
9. (первое распределение)
...
| `Одобренные перемещения` | Слой 3в: фактически использованные согласованные перемещения (лист появляется если они были) |
...
| `Дефицит` | Материалы к закупу: все поля для формирования заявки |
...
> **Полное руководство по всем вариантам развёртывания** — в отдельном документе:
> [`docs/РАЗВЕРТЫВАНИЕ.md`](docs/РАЗВЕРТЫВАНИЕ.md)
>
> Там описаны: локальный запуск (Windows/Linux/macOS), Ubuntu/Debian-сервер,
> RHEL/CentOS, **Docker Compose**, Windows Server, облако.

### Текущий production-сервер (Oracle Cloud)
...
- [x] Защита от одновременных запусков расчёта (threading.Lock, HTTP 409)
- [x] Руководство по развёртыванию — все варианты (Ubuntu, RHEL, Docker, Windows Server)
