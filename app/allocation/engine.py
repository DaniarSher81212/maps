...
class AllocationEngine:
    ...
    def _process_requirement(
        ...
        # ── Пропускаем приостановленные и отменённые работы ──
        # cancelled — работа отменена, материалы не нужны.
        # on_hold   — работа заморожена; резервировать склад под неё не нужно,
        #             иначе заблокируем материалы для активных работ.
        # Когда on_hold снова станет active — в следующем запуске получит распределение.
        if work.status in ("cancelled", "on_hold"):
            logger.debug("  [пропуск] %s — статус=%s", work.kod_raboty, work.status)
            return
        ...
        # Работа считается завершённой если статус = "completed"/"завершена"
        # ИЛИ дата окончания уже в прошлом.
        # Для завершённых работ обрабатываем только Слои 1 и 2 —
        # склад, поставки и дефицит пропускаем (закупать уже незачем).
        is_completed = (
        ...
        if is_completed:
            ...
            logger.debug(
                "  [завершена] %s — пропускаем слои 3-5 (статус=%s, дата=%s)",
                work.kod_raboty, work.status, work.data_okonchaniya,
            )
